...
export function CanvasHeader({ onNavigate, activeSection, darkMode, onToggleDarkMode }: CanvasHeaderProps) {
  ...
  return (
    <header className="anim-slide-down fixed top-0 left-0 right-0 z-50 bg-white/90 dark:bg-[#0D0D0E]/90 backdrop-blur-md border-b border-neutral-200 dark:border-neutral-800 select-none">
      {/* Top Menu Bar */}
      <div className="relative flex items-center justify-between px-4 py-2 border-b border-neutral-200/60 dark:border-neutral-800/60 text-xs font-mono">
        {/* Left Nav */}
          ...
          <nav className="hidden sm:flex items-center gap-1">
            {navItems.map((item) => (
              <button
                ...
                className={`px-3 py-1 rounded font-bold uppercase transition-all cursor-pointer ${activeSection === item.id
        ...
        <div className="flex items-center gap-2">
          {/* Language Toggle */}
          <div className="hidden sm:flex items-center text-[11px] font-bold tracking-wider border border-neutral-200 dark:border-neutral-800 rounded px-2 py-0.5 text-neutral-500">
            <button
              onClick={() => setLang("en")}
              className={`transition-colors cursor-pointer ${lang === "en" ? "text-black dark:text-white" : "opacity-40 hover:opacity-70"}`}
            >
              EN
            </button>
